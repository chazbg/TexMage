...
imageViewer
  = do -- the main frame, we use 'fullRepaintOnResize' to prevent flicker on resize
       ...
       sRGBProcessMode  <- menuItem processModeMenu [text := "&sRGB to linear",   help := "Converts the image from sRGB to linear space"]
       dummyProcessMode <- menuItem processModeMenu [text := "&Dummy processing", help := "Divides each channel's intensity by 2"]
       ...
       set f [layout                     := createFrameLayout p1 p2,
              ...
              on (menu sRGBProcessMode)  := set vProcess [value := sRGBtoLinear],
              on (menu dummyProcessMode) := set vProcess [value := dummyProcess], 
